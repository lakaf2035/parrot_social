...
class UpdatePostForm extends \moodleform {
    ...
    public function definition() {
        ...
        $attrs = array_merge($attrs, array('callback' => 'postUpdatedCallback'));
        $m_form->setAttributes($attrs);

        $m_form->addElement('hidden', 'action', 'update_post');
        ...
        $m_form->addElement('editor', 'text', get_string('post_text', 'local_parrot_social'), "id='update_form" . ($this->_customdata['id'] ?? '') . "' cols='30' rows='10'",  array(
            'noclean' => true,
            'trusttext' => true,
            'context' => $context,
